...
def _parse_block_node_linear(summary_tr, details_tr, date_str) -> Optional[Dict]:
    ...
    # If we haven't found a date yet, we can't fully parse a slot.
    # But usually date comes first.
    if not date_str:
        # Fallback: try to see if THIS row has the date (already handled in loop, but...)
        pass
...
def _extract_items_linear(html: str) -> List[Dict]:
    ...
    while i < len(rows):
        ...
        # Note: sometimes calendarBox is in the same row as content? 
        # Based on previous logic `_row_date_str`, it seemed to search *upwards*.
        # We will check if this row defines a new date.
